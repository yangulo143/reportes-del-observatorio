...
tryCatch({
  raw_data <- read_csv("etnicidad.csv", show_col_types = FALSE)
}, error = function(e) {
  stop("No se encontró el archivo 'etnicidad.csv'. Asegúrate de que esté en la carpeta del proyecto.")
})
...
Este informe analiza un total de **`r nrow(data_processed)`** documentos científicos publicados hasta el año **2025**, que abarcan aspectos sobre desenlaces en salud en diferentes grupos étnicos y son publicados por al menos un autor que tiene filiación en Perú.
